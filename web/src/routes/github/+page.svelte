...
<div class="h-full mx-auto mt-5">
	...
	<div class="card p-4 ml-4 mb-4" style="width: 400px;">
		{#if repo.language}
			<p class="text-center mb-1 text-cyan-400">{repo.language}</p>
		{:else}
		<br>
		{/if}
		<h1 class="text-center">{repo.name}</h1>
		<div class="justify-center text-center mt-4 mb-4">
			<a class="btn bg-primary-hover-token border-2 border-cyan-400 text-cyan-400" href="{repo.html_url}" target="_blank">
				<IconBrandGithub />
				<p class="uppercase">open</p>
			</a>
			<button use:clipboard="{repo.ssh_url}" class="btn bg-primary-hover-token border-2 border-cyan-400 text-cyan-400">
				<IconBrandGit />
				<!-- {copied ? 'Copied 👍' : 'Copy'} -->
